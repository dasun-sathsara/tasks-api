...
export const build = (additionalArguments = []) => {
    ...
    const { code } = shell.exec(`node ${pathToTsc} ${additionalArguments.join(' ')}`);
...
export const bundle = () => {
    ...
    // before bundling.
    build(['--noEmit']);
    const pathToEsbuild = ['node_modules', 'esbuild', 'bin', 'esbuild'].join(path.sep);
...
export const format = (additionalArguments) => {
    const pathToPrettier = ['node_modules', 'prettier', 'bin-prettier'].join(path.sep);
    const { code } = shell.exec(`node ${pathToPrettier} . ${additionalArguments}`);
    if (code !== 0) {
...
export const lint = (additionalArguments) => {
    const pathToEslint = ['node_modules', 'eslint', 'bin', 'eslint.js'].join(path.sep);
